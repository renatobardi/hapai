...
async fn main() -> Result<()> {
    ...
    println!("Flash model (Brief/Stories): {}", flash_model);
    println!("Pro model   (PRD/Arch):      {}", pro_model);
    println!("Starting 10x BriefArtifact calls...\n");

    let flash_client = GeminiClient::new(api_key.clone(), flash_model.clone())?;
    let pro_client = GeminiClient::new(api_key, pro_model.clone())?;
    ...
    // --- Phase 2: PrdArtifact, 5 runs (gemini-2.5-pro) ---
    println!("Starting 5x PrdArtifact calls (pro model)...\n");
    ...
    // --- Phase 3: Retry loop test (flash model) ---
    println!("Testing retry loop (forced schema error → re-prompt)...\n");
    let retry_call = retry_loop_test(&flash_client, &brief_schema).await;
