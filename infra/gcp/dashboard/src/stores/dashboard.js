...
export const dashboardStore = writable({ loading: false, error: null, stats: null, timeline: null, hooks: null, denials: null, tools: null, projects: null })
...
export async function loadDashboard(idToken) {
  ...
  try {
    const [stats, timeline, hooks, denials, tools, projects] = await Promise.all([
      ...
      queryBQ('denials', idToken), queryBQ('tools', idToken), queryBQ('projects', idToken),
    ])
    dashboardStore.set({ loading: false, error: null, stats, timeline, hooks, denials, tools, projects })
  } catch (err) {
