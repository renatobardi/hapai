<script>
  ...
  function build() {
    ...
    chart = new Chart(canvas, { type:'bar', data:{ labels:s.map(([d])=>d), datasets:[
      {label:T('charts.labels.denials'),data:s.map(([,v])=>v.denies),backgroundColor:'rgba(192,57,43,0.85)',borderRadius:0,borderWidth:0},
      {label:T('charts.labels.warnings'),data:s.map(([,v])=>v.warns),backgroundColor:'rgba(230,126,34,0.7)',borderRadius:0,borderWidth:0},
...
<div class="card"><div class="card-title">{$t('charts.timeline')}</div><div class="w"><canvas bind:this={canvas}></canvas></div></div>
