<script>
  ...
  function build() {
    ...
    chart = new Chart(canvas, { type:'bar', data:{ labels:data.map(d=>d.project.split('/').pop()), datasets:[{label:'Denials',data:data.map(d=>d.count),backgroundColor:'rgba(39,174,96,0.85)',borderRadius:0,borderWidth:0}] },
...
<div class="card"><div class="card-title">Projects</div><div class="w"><canvas bind:this={canvas}></canvas></div></div>
